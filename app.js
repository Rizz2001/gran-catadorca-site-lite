...
async function obtenerArchivosExternos() {
    try {
        ...
        if (resTasa.ok) { let texto = await resTasa.text(); tasaOficial = parseFloat(texto.trim().replace(',', '.')); }
    } catch (error) { console.log("Error leyendo tasa.txt"); }
    ...
    try {
        ...
        if (resRec.ok) { let textoRec = await resRec.text(); codigosRecomendados = textoRec.split(/[\n,]+/).map(c => c.trim()).filter(c => c !== ""); }
    } catch (error) { console.log("No se encontró recomendados.txt"); }

    try {
        let resDisp = await fetch('disponibles.txt?v=' + new Date().getTime());
        if (resDisp.ok) { let textoDisp = await resDisp.text(); siempreDisponibles = textoDisp.split(/[\n,]+/).map(c => c.trim()).filter(c => c !== ""); }
...
function imgFallback(imgElement, codigoProducto) {
    ...
    if (attempts < formatos.length) { 
        ...
        imgElement.src = 'logo.png'; imgElement.onerror = null; 
    }
}
...
async function cargarInventario() {
    ...
    try {
        ...
        document.getElementById('lista-productos').innerHTML = '<p style="text-align:center; grid-column:span 2; color: red; margin-top: 20px;">Error al leer inventario.</p>'; 
    }
}
...
function mostrarSugerencias(q) {
    ...
    coincidencias.forEach(p => {
        ...
        div.innerHTML = `<img src="img/${p.codigo}.webp" onerror="imgFallback(this, '${p.codigo}')"><span>${p.Nombre}</span>`;
        div.onclick = () => { document.getElementById('buscador').value = p.Nombre; cerrarSugerencias(); aplicarFiltros(); };
...
function irInicio() { 
    ...
    if(btnLicores) filtrarCategoria('LICORES', btnLicores); else filtrarCategoria('Todos', document.querySelectorAll('.cat-btn')[0]);
}
...
function abrirPerfil() { 
    ...
    if(hist.length === 0) { listCont.innerHTML = '<p style="font-size:12px; color:gray; text-align:center;">Aún no tienes pedidos registrados.</p>'; } 
        ...
        hist.forEach((ped, index) => {
            ...
            listCont.innerHTML += `
                <div style="border:1px solid var(--borde-color); padding:10px; border-radius:12px; margin-bottom:10px;">
                    ...
                    <button onclick="repetirPedido(${index})" style="background:var(--azul-rey); color:white; border:none; padding:8px; width:100%; border-radius:8px; font-size:11px; font-weight:bold; cursor:pointer;"><i class="fa-solid fa-rotate-right"></i> Repetir este pedido</button>
                </div>
            `;
        });
    }
...
function renderizarCarrito() {
    ...
    if(Object.keys(carrito).length === 0) {
        lista.innerHTML = `
            <div style="text-align: center; padding: 50px 20px; color: var(--texto-claro);">
                <i class="fa-solid fa-cart-shopping" style="font-size: 60px; opacity: 0.2; margin-bottom: 20px;"></i>
                <h3 style="color: var(--texto-oscuro); font-size: 16px; font-weight: bold;">Tu carrito está vacío</h3>
                <p style="font-size: 13px; margin-top: 5px;">Agrega unas botellas para empezar.</p>
                <button onclick="cerrarModal('modal-cart', 'nav-home')" class="btn-enviar" style="width: auto; padding: 10px 25px; margin-top: 20px; display: inline-block;">Ir a la tienda</button>
            </div>`;
